# generate_graph_images.py
# Generates graph snapshots for months 1, 6, 12 for the first N users.
# Output files: graphs/graph_user{user_id}_month{month}.png
...
N_USERS = 50               # <-- change this (e.g., 20, 100)
...
def build_financial_graph(users_df, monthly_df, user_id, month):
    ...
    G.graph["user_id"] = int(user_id)
    G.graph["month"] = int(month)
    return G
...
# First N users
user_ids = users["user_id"].tolist()[:N_USERS]
...
for uid in user_ids:
    for m in MONTHS:
        ...
        draw_and_save_graph(G, out_path, f"User {uid} — Month {m}")
        count += 1
